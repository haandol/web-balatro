...
- 라운드 점수 / 목표 점수 표시
- **실시간 핸드 프리뷰**: 카드 선택 시점에 즉시 포커 핸드 유형 + 예상 점수를 표시 (제출 전 미리 확인). `evaluateHand()` + `calculateHandScore()`를 computed로 선택된 카드에 대해 실행
- 마지막 플레이 결과 (핸드 유형 + 획득 점수) 표시
...
| 점수 / 목표 표시 + 프로그레스 바 | ✅ 완료 |
| 카드 선택 시 실시간 핸드 프리뷰 (유형 + 예상 점수) | ✅ 완료 |
| 점수 계산 애니메이션 | ❌ 미구현 |
| 카드 이동 애니메이션 | ❌ 미구현 |
