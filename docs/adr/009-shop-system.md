...
| 항목 | 상태 |
|---|---|
| 블라인드 클리어 후 상점이 표시된다 | ✅ 완료 |
| 자금이 충분할 때만 아이템을 구매할 수 있다 | ✅ 완료 |
| 리롤 시 새 아이템으로 교체되고 비용이 차감된다 | ✅ 완료 |
| 리롤 비용이 동일 상점 내에서 점진적으로 증가한다 | ✅ 완료 (+$1/리롤) |
| 부스터 팩 구매 시 카드 선택 화면이 표시된다 | ❌ 미구현 (후순위) |
| "Next Round" 버튼으로 다음 블라인드 선택 화면으로 이동한다 | ✅ 완료 |
